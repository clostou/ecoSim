...
在食物链中，植物属于生产者，是整个生态系统的基石。植物会在固定的位置**随时间自然生长**，当生长到最大值时则会向附近的空白地段扩张（产生新的植物实体）。不同生长阶段的植物拥有不同的体积（或空间密度），这会影响视线的穿透，因此动物可以藏匿其中。初始时会有一些植物分布于环境的某些位置，正常情况下植物会从这些位置逐渐生长蔓延，但实际上没有植物的空地也有小概率生长出植物，这类似于自然界中种子的**风传播**。
...
classDiagram
  ...
  class Creature ["有机生物 Creature"]{
    +int reserve, breed, age
    +update(delta_reserve) void
    +isAlive() bool
    +isBreed() bool
  }
...
生物通过关键变量`reserve`来表达单个个体储存在体内的能量，该值可以通过实例的`update`函数控制每个时间步的增减：增加对应光合作用或觅食，减少对应被食用或腐败。另外还引入了能量传递系数`TRANSFER_COEF`来表征能量沿食物链逐级衰减的过程。当能量输入，或生物繁殖、死亡时，会有固定比例的能量被耗散。
